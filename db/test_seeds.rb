...
ga = EducationSubject.create!(school: school1, name: 'Gardening',  short_name: 'GA')
pl = EducationSubject.create!(school: school2, name: 'Plumbing',   short_name: 'PL')
to = EducationSubject.create!(school: school2, name: 'Touristing', short_name: 'TO')

# John is manager and teacher on School1
john  = Person.create!(first_name: 'John',  last_name: 'Maker',  contact: {email: 'john@bak.er'}) do |person|
...
sofia = Person.create!(first_name: 'Sofia', last_name: 'Gentle', contact: {email: 'sofia@gent.le'}) do |person|
  person.create_as_manager! organisation: school1
  person.generate_login! (person.first_name + person.last_name).downcase
end

# # Fred is teacher on School1
# fred = Person.create!(first_name: 'Fred', last_name: 'Mecky', contact: {email: 'meck@mail.com'}) do |person|
#   person.create_as_teacher! organisation: school1
#   person.generate_login! (person.first_name + person.last_name).downcase
# end
